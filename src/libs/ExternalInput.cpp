...
namespace erizo {
  ...
  int ExternalInput::init(){
    ...
    // av_sdp_create(&context_, 1, sdpBuffer_, 20000);

    // std::string sdp = std::string(sdpBuffer_, 20000);

    // ELOG_DEBUG("SDP %s", sdp.c_str() );
  ...
  void ExternalInput::receiveRtpData(unsigned char*rtpdata, int len) {
    if (videoSink_!=NULL){
      ELOG_DEBUG("Deliver RTP packet %d", len);
    ...
    while(av_read_frame(context_,&avpacket_)>=0&& running_==true){
      ...
      if (needTranscoding_){
        ...
        if(avpacket_.stream_index == video_stream_index_){//packet is video
          ...
          ELOG_DEBUG("AUDIO %d %ld",  avpacket_.size, avpacket_.pts);
