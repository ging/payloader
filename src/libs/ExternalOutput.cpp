...
ExternalOutput::ExternalOutput(const std::string& outputUrl) : audioQueue_(5.0, 10.0), videoQueue_(5.0, 10.0), inited_(false), video_stream_(NULL), audio_stream_(NULL),
...
void ExternalOutput::writeRawData(char* buf, int len, int streamIndex){

	ELOG_DEBUG("WRITE DATA in %d", streamIndex);
...
void ExternalOutput::writeAudioData(char* buf, int len){
    ...
    ELOG_DEBUG("AUDIO CODEC %d",  context_->oformat->audio_codec)
...
ELOG_DEBUG("WRITE AUDIO DATA 4 %lld %lld %d", timestampToWrite, audioOffsetMsec_, audio_stream_->time_base.den);
...
ELOG_DEBUG("WRITE AUDIO DATA 5 %d", avpkt.size);
...
int ExternalOutput::deliverRtpData_(char* buf, int len) {
    return 0;
}
...
void ExternalOutput::queueData(char* buffer, int length, packetType type){
    ...
    if (type == VIDEO_PACKET){
        ...
        if(this->audioOffsetMsec_ == -1) {
       	...
       	ELOG_DEBUG("WRITE AUDIO");
       	this->writeAudioData(buffer, length);
