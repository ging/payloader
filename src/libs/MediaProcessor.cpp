...
namespace erizo {
  ...
  int InputProcessor::deliverRtpData_(char* buf, int len) {
    RtpHeader* head = reinterpret_cast<RtpHeader*>(reinterpret_cast<unsigned char*>(buf));
    ELOG_DEBUG("PACKAGE HEADER %d", head->getPayloadType());
    if (head->getPayloadType() == 100) {
      this->deliverVideoData_(buf, len);
    } else {
  ...
  void OutputProcessor::receiveRtpData(char* buf, int len) {
  }
...
//      head.setTimestamp(pts*8);
      ELOG_DEBUG("Set timestamp %ld", av_rescale(pts, mediaInfo.audioCodec.sampleRate,1000))
